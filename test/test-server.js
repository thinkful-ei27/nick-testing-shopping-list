...
describe('Recipes', function(){
  ...
  it('should GET items upon GET', function(){
    return chai
      .request(app)
      .get('/recipes')
      .then(function(res){
  ...
  it('should add an item on post', function(){
    const testItem = {name: 'Meatloaf', ingredients: ['Meat', 'More Meat']};
    return chai 
      .request(app)
      .post('/recipes')
      .send(testItem)
  ...
  it('Should Replace an Item on Put', function(){
    const updatedTestItem = {name: 'Pizza', ingredients: ['Phone Call', 'Money']};
    return chai 
      .request(app)
      .get('/recipes')
      .then(function(res){
        updatedTestItem.id = res.body[0].id;
        return chai
          .request(app)
          .put(`/recipes/${updatedTestItem.id}`)
          .send(updatedTestItem);
      })
      .then(function(res){
        expect(res).to.have.status(200);
        expect(res).to.be.json;
        expect(res.body).to.be.a('object');
        expect(res.body).to.deep.equal(updatedTestItem);
      });    
  });
});
